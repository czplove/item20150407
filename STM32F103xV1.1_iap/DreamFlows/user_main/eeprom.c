...
void test_SYS_VERSION(void)
{

    if(*(__IO uint16_t*)(APP_CONFIG3_ADDR) != STM32_SYS_VERSION_NUM)
    {
        FLASH_Unlock();						//½㌸
				FLASH_ErasePage(APP_CONFIG3_ADDR);//²s�ɇ�		
				FLASH_ProgramHalfWord(APP_CONFIG3_ADDR,STM32_SYS_VERSION_NUM);	//-²»ŜՋѐ¿ʒՔ̐΍
				FLASH_Lock();//ʏ̸
    }
}

void run_in_iap(void)
{
    ...
    FLASH_ClearFlag(FLASH_FLAG_EOP | FLASH_FLAG_PGERR | FLASH_FLAG_WRPRTERR);
		...
		FLASHStatus =	FLASH_ErasePage(APP_CONFIG_ADDR);//²s�ɇ�		//-FLASH_ProgramHalfWord(APP_CONFIG_ADDR,0x0000);	//-²»ŜՋѐ¿ʒՔ̐΍
        while(FLASHStatus != FLASH_COMPLETE)
          FLASH_ErasePage(APP_CONFIG_ADDR);
		FLASHStatus =	FLASH_ErasePage(APP_CONFIG1_ADDR);//²s�ɇ�		//-FLASH_ProgramHalfWord(APP_CONFIG2_ADDR,0x0000);	//-Ǵ¶¯¸´׆
        //-FLASH_ProgramHalfWord(APP_CONFIG1_ADDR,0x0000);	//-Ǵ¶¯¸´׆
        while(FLASHStatus != FLASH_COMPLETE)
