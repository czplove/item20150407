...
static int escore_uart_setup_high_bw_intf(struct escore_priv *escore)
{
 ...
 {  
     unsigned int l,temp_in,temp_out,temp_data,temp_data1;  
     struct cycle_buffer *temp_fifo;
     
     temp_fifo = fifo;
     temp_in = temp_fifo->in;
     temp_out = temp_fifo->out;
     temp_data = (temp_in & (RX_BUFF_SIZE - 1));
     //-temp_data1 = RX_BUFF_SIZE - temp_in + temp_out;
       
     len = min(len, RX_BUFF_SIZE - temp_in + temp_out);  
     l = min(len, RX_BUFF_SIZE - temp_data);       
     memcpy(temp_fifo->buf + temp_data, buf, l);  
     memcpy(temp_fifo->buf, buf + l, len - l);  
     temp_fifo->in += len;  
     return len;  
 }  
