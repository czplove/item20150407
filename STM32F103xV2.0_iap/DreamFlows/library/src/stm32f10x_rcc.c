...
void RCC_HSEConfig(u32 RCC_HSE)
{
  ...
  assert_param(IS_RCC_HSE(RCC_HSE));  //-本条指令其实可以不需要

  /* Reset HSEON and HSEBYP bits before configuring the HSE ------------------*/
  /* Reset HSEON bit */
  RCC->CR &= CR_HSEON_Reset;  //-不一定要找到什么说明性的文章,现实中能用就行,不对再找原因
...
void RCC_PLLCmd(FunctionalState NewState)
{
  ...
  *(vu32 *) CR_PLLON_BB = (u32)NewState;  //-这里使用的应该是位寻址,很值得学习
}
...
void RCC_APB2PeriphClockCmd(u32 RCC_APB2Periph, FunctionalState NewState)
{
  ...
  if (NewState != DISABLE)
  {
    RCC->APB2ENR |= RCC_APB2Periph; //-简单的配置就是把需要的位置一
  }
  else
  {
...
FlagStatus RCC_GetFlagStatus(u8 RCC_FLAG)
{
  ...
  tmp = RCC_FLAG >> 5;  //-不同类型的需要检查不同状态寄存器
