...
int I2C1_WriteByte(uint8_t DeviceAddress, uint8_t *Data, uint8_t len)  
{
    ...
    struct i2c_client client;
	  ...
	  client.addr = DeviceAddress;                   // 将从机地址写入数据包pa12201001的地址就是0x3C 
    client.flags = 0;
    client.num = 2;		//-硬件端口号2,I2C2
    
    
    res = i2c_write_reg_Buffer(&client, Data, len);
	  if(res <= 0)
	  {//-读值成功的话就继续判断      
	 		return 0;
		}
...
uint8_t I2C1_ReadByte(uint8_t DeviceAddress, u8 *buf, u8 len)  
{
    ...
    res = i2c_read_data_Buffer(&client, len, buf);
	  if(res <= 0)
	  {//-读值成功的话就继续判断      
	 		return 0;
		}
...
int escore_i2c_cmd(struct escore_priv *escore, u32 cmd, u32 *resp)
{
	...
	do {
		...
		*resp = be32_to_cpu(rv);
		if (err) {
			;//-printf(" escore_i2c_read() failure\n");
