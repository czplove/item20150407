...
int process_es70x_interrupt(struct escore_priv *es705)
{
  ...
  es705_event_type = event_type;
  es705_TO_UART2_FLAG = 2;    //-发送目前唤醒语句的编号
	...
	if (voice_sense->cvs_preset != 0xFFFF && voice_sense->cvs_preset != 0) {
		...
		if (rc) 	//-再一次配置中断失败了怎么办,是否不能再次唤醒了?
		{
			//-printf("reconfig interrupt failed rc = %d\n",rc);
			return NOTIFY_DONE;
		}
	}
...
static int es705_start_int_osc(void)
{
	...
	do {
		...
		if (rc < 0) {
			//-printf("OSC Measure Read Status fail\n");
			break;
		}
		//-printf("OSC Measure Status = 0x%04x\n", rc);
	} while (rc && --retry);

	if (rc > 0) {
		//-printf("Unexpected OSC Measure Status = 0x%04x\n", rc);
		;//-printf("Can't switch to Low Power Mode\n");
	}
	...
	while (next_power_state != escore->escore_power_state) {
		...
		case ES_SET_POWER_STATE_VS_OVERLAY:
			...
			if (reconfig_intr) {
				...
				if (rc < 0) {
					//-printf(" Interrupt config failed :%d\n",rc);
					goto es705_power_transition_exit;
				}
			}
			break;
...
static int convert_model_to_download_format(char *name, int num)
{
   ...
   if(es705_mode_file == 1)   	
	 		rc = f_open(&final_fd, "TKW1.BIN",FA_WRITE | FA_CREATE_ALWAYS); 	//以读的方式打开文件
	 else if(es705_mode_file == 2)		
	 	  rc = f_open(&final_fd, "TKW2.BIN",FA_WRITE | FA_CREATE_ALWAYS); 	//以读的方式打开文件
	 else if(es705_mode_file == 3)		
	 	  rc = f_open(&final_fd, "TKW3.BIN",FA_WRITE | FA_CREATE_ALWAYS); 	//以读的方式打开文件
	 else if(es705_mode_file == 4)		
	 	  rc = f_open(&final_fd, "TKW4.BIN",FA_WRITE | FA_CREATE_ALWAYS); 	//以读的方式打开文件
	 else if(es705_mode_file == 5)		
	 	  rc = f_open(&final_fd, "TKW5.BIN",FA_WRITE | FA_CREATE_ALWAYS); 	//以读的方式打开文件	  	  	  
	 //-if(rc < 0)
...
int set_es705_to_training_mode(void)
{
    ...
    es705_TO_UART2_FLAG = 1;    //-周期上送说明现在所处的状态
		
    es705_msleep_time = cticks_ms;
    while(Judge_Time_In_MainLoop(es705_msleep_time,1000)!=YES) 
    {
      CDT9702_Main();   //-增加串口处理,保证串口可靠
      LDISP_Main();
    }
    if( count  == 4)
			break;
    if(es705_mode == 0)
      break;
		//-msleep(1000);    //-这里的延时,占用了CPU资源现在需要释放出来
    if(status >= 6)
    {
    ...
    //-学习超时退出
    if(Judge_LongTime_In_MainLoop(es705_training_wait_time,146)==YES) //5min
    {
  ...
  //-退出进程后需要把状态值复位
  //-es705_training_count = 0;   //-这里不能立即清0,还没有送出去
if(count  == 4)  //-只有学习成功了才有必要下载否则没有必要进行
{
  if(es705_mode_file == 1)
  {  
    copy_training_model_file_from_es705(escore,"TRAIN0.BIN");
    convert_model_to_download_format("TRAIN0.BIN", 0);
  }
  else if(es705_mode_file == 2)
  {  
    copy_training_model_file_from_es705(escore,"TRAIN1.BIN");
    convert_model_to_download_format("TRAIN1.BIN", 1);
  }
  else if(es705_mode_file == 3)
  {  
    copy_training_model_file_from_es705(escore,"TRAIN2.BIN");
    convert_model_to_download_format("TRAIN2.BIN", 2);
  }
  else if(es705_mode_file == 4)
  {  
    copy_training_model_file_from_es705(escore,"TRAIN3.BIN");
    convert_model_to_download_format("TRAIN3.BIN", 3);
  }
  else if(es705_mode_file == 5)
  {  
    copy_training_model_file_from_es705(escore,"TRAIN4.BIN");
    convert_model_to_download_format("TRAIN4.BIN", 4);
  }  
}  
