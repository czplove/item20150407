...
void DMA_DeInit(DMA_Channel_TypeDef* DMAy_Channelx)
{
  ...
  DMAy_Channelx->CCR &= CCR_ENABLE_Reset; //-使通道不工作,可能是为了下面配置

  /* Reset DMAy Channelx control register */
  DMAy_Channelx->CCR  = 0;  //-其实和上面的是一样的
  
  /* Reset DMAy Channelx remaining bytes register */
  DMAy_Channelx->CNDTR = 0; //-传输数量寄存器,指示剩余的待传输字节数目
  
  /* Reset DMAy Channelx peripheral address register */
  DMAy_Channelx->CPAR  = 0; //-外设地址寄存器,外设数据寄存器的基地址，作为数据传输的源或目标
  
  /* Reset DMAy Channelx memory address register */
  DMAy_Channelx->CMAR = 0;  //-存储器地址寄存器,存储器地址作为数据传输的源或目标

  switch (*(u32*)&DMAy_Channelx)
  {
    case DMA1_Channel1_BASE:
      /* Reset interrupt pending bits for DMA1 Channel1 */
      DMA1->IFCR |= DMA1_Channel1_IT_Mask;  //-中断标志清除寄存器,写1清除DMA_ISR寄存器中的对应的标志
      break;
