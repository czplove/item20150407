...
static int escore_vs_sleep(struct escore_priv *escore)
{
	...
	if (rc) {
		//-printf(" datablock write fail rc = %d\n",rc);
		goto vs_sleep_err;
	}
...
static int escore_cvq_sleep_thread(void *ptr)
{
	...
	if (rc != -EABORT)
	{
				//-printf("escore_cvq_sleep_thread_exit\n");
		goto escore_cvq_sleep_thread_exit;


	}

				//-printf("escore_cvq_sleep_thread Normal\n");
	/* change power state to Normal*/
	cmd = ((u32)ES_SET_POWER_STATE << 16) | ES_SET_POWER_STATE_NORMAL;
	rc = escore->bus.ops.cmd(escore, cmd, &rsp);
	if (rc) {
		//-printf("Power State Normal failed rc = %d\n",rc);
	}
...
int escore_vs_write_bkg_and_keywords(struct escore_priv *escore)
{
	...
	if (rc) 
  {
	...
	//-rc = escore_datablock_write(escore,"OEM.BIN");    //-这个"你好 中心"是系统自带的,如果不需要可以注释掉,然后下面多一个用户定义的
	if (rc) 
  {
	...
	//-if(es705_mode_file == 1)
		 rc = escore_datablock_write(escore,"TKW1.BIN");  //-下载了文件才有唤醒能力
	//-else if(es705_mode_file == 2)
		 rc = escore_datablock_write(escore,"TKW2.BIN");
	//-else if(es705_mode_file == 3)
		 rc = escore_datablock_write(escore,"TKW3.BIN");
	//-else if(es705_mode_file == 4)
		 rc = escore_datablock_write(escore,"TKW4.BIN");
	//-else if(es705_mode_file == 5)
		 rc = escore_datablock_write(escore,"TKW5.BIN");	 	 	 
		...
		void *dev)
{
	...
	if (voice_sense->cvs_preset != 0xFFFF && voice_sense->cvs_preset != 0) {
		...
		if (rc) {
			//-printf("%s() reconfig interrupt failed rc = %d\n",rc);
			return NOTIFY_DONE;
		}
	}
...
int escore_vs_load(struct escore_priv *escore)
{
	...
	if (!escore->boot_ops.setup || !escore->boot_ops.finish) {
		//-printf(" boot setup or finish function undefined\n");
		rc = -EIO;
